...
There are separate CSV files for each sampling date with the same format and file naming convention. Our goal is to have the data from all 50 files into the same data frame, and add the date metadata from the filename into the table (is not currently).
...
Load the `fs` package to work with the file system. Create a vector of files with `dir_ls()`
...
~~~
{:title="{{ site.handouts[0] }}" .text-document}


===

`fs` path objects are character vectors but a little smarter. Specialized functions can extract or retrieve parts of path.  
...
~~~r
> pg_list <- map(.x = penguin_files, .f = read_csv)
~~~
{:title="Console" .input}
...
~~~r
> pg_files <- map(.x = penguin_files, ~read_csv(.x))
~~~
{:title="Console" .input}
...
Hint: Check out the `split` or (experimental) `group_split` function for creating the list. 
